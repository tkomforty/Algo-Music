...
let numGrains = 100;
let gravity;
let t = 0; // Time variable for evolving patternsnote
let particleSystem;
...
function setup() {
  ...
  for (let i = 0; i < numLayers; i++) {
    ...
    let hue = map(i, 0, numLayers - 1, 20, 100);
    let sat = map(i, 0, numLayers - 1, 0, 70);
    let bri = map(i, 0, numLayers - 1, 50, 100);
    let sandColor = color(hue, sat, bri);
    let sandTexture = random(0.003, 0.01); // Slightly lower texture values for smoother flow
...
function initializeParticles() {
  ...
  for (let i = 0; i < edgeCount; i++) {
    ...
    particleSystem.addParticle(x, y, color(hue, 70, 90, 100));
  }
...
function draw() {
  t += 0.005; // Increment time for evolving patterns
  updateGravity();
  ...
  if (audioInitialized) {
    ...
    grainActivityLevel = lerp(
      grainActivityLevel,
      constrain(grainActivityLevel, 0.1, 0.9),
      0.05
    );
    ...
    if (
      frameCount % 360 === 0 ||
      (gravity.mag() > 0.05 && frameCount % 120 === 0)
    ) {
      changeRootNote();

      // Create an attractor at a position based on the new root note
      if (random() < 0.7) {
        const rootFreqNormalized = map(
          rootFrequency,
          possibleRoots[0],
          possibleRoots[possibleRoots.length - 1],
          0,
          1
        );

        const attractorX = map(
          rootFreqNormalized,
          0,
          1,
          width * 0.2,
        ...
        if (particleSystem.attractors.length < 5) {
          particleSystem.attractors.push({
            ...
            strength: random(0.1, 0.5) * (random() < 0.5 ? -1 : 1),
            radius: random(100, 250),
            moving: random() < 0.5,
            velocity: p5.Vector.random2D().mult(random(0.05, 1.5)),
          });
        } else {
          // Update a random attractor
          const idx = floor(random(particleSystem.attractors.length));
          particleSystem.attractors[idx].pos = createVector(
            attractorX,
            attractorY
          );
          particleSystem.attractors[idx].strength =
            random(0.1, 0.4) * (random() < 0.5 ? -1 : 1);
        }
      }
    }
    ...
    if (frameCount % beatLength === 0) {
      ...
      if (frameCount % (beatLength * timeSignature) === 0) {
        if (random() < 0.7) {
          const velocity = map(grainActivityLevel, 0, 1, 0.02, 0.4);
          ...
          particleSystem.addParticlesOnBeat(
            burstX,
            burstY,
            floor(random(5, 15)),
      ...
      // Sometimes on beat 3
        ...
        if (random() < 0.4) {
          const velocity = map(grainActivityLevel, 0, 1, 0.15, 0.55);
          ...
          if (random() < 0.5) {
            ...
            particleSystem.addParticlesOnBeat(
              ...
              burstY,
              floor(random(3, 8)),
              color(random(100, 200), random(100, 200), random(200, 255), 75)
            );
          }
        }
      }
      // Rarely on other beats
        ...
        const velocity = map(grainActivityLevel, 0, 1, 0.1, 0.53);
    ...
    if (particleSystem.particles.length > 0 && frameCount % 30 === 0) {
      const noteProb = map(particleSystem.particles.length, 0, 50, 0.03, 0.5);
      if (random() < noteProb) {
        ...
        if (random() < 0.5 && particleSystem.particles.length > 5) {
          ...
          for (let i = 0; i < min(8, particleSystem.particles.length); i++) {
            const p1 =
              particleSystem.particles[
                floor(random(particleSystem.particles.length))
              ];
            const p2 =
              particleSystem.particles[
                floor(random(particleSystem.particles.length))
              ];
            rect(p1.pos.x, p1.pos.y, 100, 100);
          }
        }
      }
    }
    ...
    noteSpacing = map(grainActivityLevel, 0, 1, 500, 2500);
  } else {
...
function updateGravity() {
  ...
  let angle = noise(t * 0.5) * TWO_PI; // Slower change in direction
  let magnitude = noise(t * 0.2 + t * 100) * random(0.0, 0.75); // Reduced force
...
function drawGradientBackground() {
  let x = 1.5;
  let color1 = color(40 + 10 * sin(t), 10, 100/x);
  let color2 = color(100 + 20 * cos(t), 20, 100/x);
...
function setMasterVolume(value) {
  ...
  if (masterGain) {
    ...
    const clampedValue = constrain(value, 0, 10);
    
    // Smoothly transition to new volume
    masterGain.gain.rampTo(clampedValue, 0.81);
...
function mouseDragged() {
  ...
  if (frameCount % 1 === 0) {
    ...
    if (random() < 0.5 && audioInitialized) {
...
function keyPressed() {
  ...
  // Add volume controls
    ...
    if (masterGain) {
      ...
      setMasterVolume(currentVolume + 0.51);
    }
  } else if (key === '-' || key === '_') {
...
function setupAudioComponents() {
  try {
    ...
    masterReverb = new Tone.Reverb({
      decay: 5,
      wet: 0.75, // Slightly lower reverb amount
      preDelay: 0.05,
    });
...
function playNote(velocity = 0.5) {
  ...
  try {
    ...
    if (velocity > 0.15) {
      ...
      const particleCount = floor(map(velocity, 0.15, 0.5, 12, 100));
      const hue = map(noteIndex, 0, scale.length - 1, 0, 360);
      const noteColor = color(hue, 80, 90, 100);
...
function changeRootNote() {
  ...
  try {
    ...
    const idx = Math.floor(
      map(gravity.heading(), -PI, PI, 0, circleOfFifths.length)
    );
...
function playChord() {
  ...
  try {
    ...
    setTimeout(() => {
      if (harmonySynth) {
        harmonySynth.triggerAttackRelease(scale[2], "2n", undefined, 0.57);
      }
    }, 50);
    ...
    setTimeout(() => {
      if (harmonySynth) {
        harmonySynth.triggerAttackRelease(scale[4], "2n", undefined, 0.86);
      }
    }, 100);
    ...
    for (let grain of this.grains) {
      ...
      if (random() < 0.001) {
        ...
        if (random() < 0.1 && audioInitialized) {
          ...
          const velocity = map(grain.vel.mag(), 0, 1, 0.08, 0.5);
...
class SandGrain {
  ...
  align() {
    ...
    if (count > 0) {
      ...
      steer.limit(0.72); // Max steering force
      return steer;
    } else {
...
class Particle {
  constructor(x, y, col) {
    ...
    this.oscillationSpeed = random(0.02, 0.076);
    ...
    this.hasTail = random() < 0.74;
    this.tailLength = this.hasTail ? floor(random(3, 17)) : 0;
    this.trail = [];
    ...
    if (this.behaviorType === 1) {
      ...
      this.lifespan = 500; // Longer lifespan
    }
  }

  applyForce(force) {
    // Apply force with mass consideration (size-based)
    let f = p5.Vector.div(force, this.baseSize / 2);
    this.acc.add(f);
  }
  ...
  respondToAudio(activityLevel) {
    ...
    this.size = this.baseSize * (10 + activityLevel);
  ...
  update() {
    ...
    if (this.behaviorType === 1) {
      ...
      let angle =
        this.orbitOffset + (frameCount - this.birthTime) * this.orbitSpeed;
    ...
    this.size =
      this.baseSize *
      (1 +
        sin(frameCount * this.oscillationSpeed) *
  ...
  display() {
    ...
    if (this.hasTail && this.trail.length > 0) {
      noFill();
      stroke(this.color, this.lifespan * 0.97);
      beginShape();
...
class ParticleSystem {
  constructor() {
    ...
    for (let i = 0; i < 3; i++) {
      this.attractors.push({
        ...
        strength: random(0.1, 0.84) * (random() < 0.5 ? -1 : 1), // Negative means repulsion
        radius: random(100, 300),
        moving: random() < 0.3,
        velocity: p5.Vector.random2D().mult(random(0.5, 1.5)),
      });
  ...
  createEmitter(x, y) {
    ...
    this.emitters.push({
      ...
      rate: random(0.02, 0.81),
      color: color(random(20, 255), random(200, 255), random(200, 255), 50),
      lifespan: 300 + random(50),
    });
  }
  ...
  notifyNote(noteFreq, velocity) {
    ...
    let respondCount = floor(
      map(velocity, 0, 1, 13, this.particles.length * 0.5)
    );
